...
export function InventoryOperations({staff,products,movements,initialWarehouseProductId}:{staff:Option[];products:Option[];movements:Movement[];initialWarehouseProductId?:string}) {
  ...
  return <div className="space-y-8">
    ...
    <section><h2 className="font-semibold">Post a stock movement</h2><p className="mt-1 text-sm text-slate-500">Every operation adds an immutable ledger entry. Corrections must be posted as adjustments.</p><div className="mt-4 grid gap-4 lg:grid-cols-3">
      <MovementForm id="add-warehouse-stock" title="Add Warehouse Stock" submit={e=>submit(e,"warehouse-in")} busy={busy==="warehouse-in"} products={products} initialProductId={initialWarehouseProductId}><input name="note" placeholder="Optional note or reference" /></MovementForm>
      <MovementForm id="record-staff-return" title="Record Staff Return" submit={e=>submit(e,"returns")} busy={busy==="returns"} products={products} staff={staff}><textarea name="reason" required minLength={3} placeholder="Reason for return" /></MovementForm>
      <MovementForm id="adjustment" title="Adjustment" submit={e=>submit(e,"adjustments")} busy={busy==="adjustments"} products={products} staff={staff} allowWarehouse><textarea name="reason" required minLength={3} placeholder="Required audit reason" /></MovementForm>
    </div></section>
